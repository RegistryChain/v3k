...
const RegistrarInputWrapper = styled.div<{ $size: 'medium' | 'extraLarge' }>(
  ({ theme, $size }) => css`
    z-index: 1;
...
export const RegistrarInputBox = forwardRef<HTMLInputElement, RegistrarInputBoxProps>(
  ({ size = 'extraLarge', input, setInput, containerRef }, ref) => {
    const { t } = useTranslation('common')
    return (
      <RegistrarInputWrapper ref={containerRef} $size={size}>
        <Input
          size={size}
          label={t('RegistrarInput.label')}
          hideLabel
          placeholder={t('RegistrarInput.placeholder')}
          value={input}
        ...
        />
      </RegistrarInputWrapper>
    )
  },
)

type FakeRegistrarInputBoxProps = {
  size?: 'medium' | 'extraLarge'
  onClick: (e: MouseEvent<HTMLInputElement>) => void
}

export const FakeRegistrarInputBox = forwardRef<HTMLInputElement, FakeRegistrarInputBoxProps>(
  ({ size = 'extraLarge', onClick }, ref) => {
    const { t } = useTranslation('common')
    return (
      <RegistrarInputWrapper $size={size}>
        <Input
          size={size}
          label={t('RegistrarInput.label')}
          hideLabel
