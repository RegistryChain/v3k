...
describe('EarnifiDialog', () => {
  ...
  it('should render', () => {
    ...
    expect(screen.queryByText('tabs.metadata.misc.bankless.title')).not.toBeInTheDocument()
  })

  it('should render dialog when open is true', () => {
    render(<EarnifiDialog {...defaultProps} />)
    const dialog = screen.getByText('tabs.metadata.misc.bankless.title')
  ...
  it('should show default error when no error message is passed', async () => {
    ...
    expect(await screen.findByText('tabs.metadata.misc.bankless.submitError')).toBeInTheDocument()
  })
  ...
  it('should clear error after error timeout', async () => {
    ...
    setTimeout(async () => {
      await waitFor(() => {
        expect(screen.findByText('tabs.metadata.misc.bankless.submitError')).not.toBeInTheDocument()
      })
    }, 3000)
  })
  ...
  it('should show success dialog when subscribe is successful', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText('tabs.metadata.misc.bankless.emailConfirmation')).toBeInTheDocument(),
    )
  })
